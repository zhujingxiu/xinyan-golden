...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function username_check($username = '',$ignore_id = 0)
	{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
		    ...
		    'last_ip' 	 => $ip_address,
		    'addtime' 	 => time(),
		    'status'     => ($manual_activation === false ? 1 : 0)
		);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, status, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function get_users_by_group_code($code,$company_id=FALSE)
	{
		$where = array();
		$where[$this->tables['groups'].'.code'] = strtolower($code);
		$where[$this->tables['users'].'.status'] = 1;
		if($company_id) {
			$where['company_id'] = $company_id;
		}
		return $this->db->select($this->tables['users'].".*,".$this->tables['groups'].".title,".$this->tables['groups'].".code",false)
			->where($where)
			->join($this->tables['users'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.id')
			->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
			->get($this->tables['users_groups']);
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time(),'last_ip'=>$this->_prepare_ip($this->input->ip_address())), array('id' => $id));
	...
	*/
	...
	public function update_group($group_id = FALSE, $group_code = FALSE, $additional_data = array())
	{
		...
		// IMPORTANT!! Third parameter was string type $title; this following code is to maintain backward compatibility
		// New projects should work with 3rd param as array
		if (is_string($additional_data)) $additional_data = array('title' => $additional_data);
	...
	**/
	...
	function get_node_by_path($path,$mode = 'auth')
	{
		...
		return $this->db->where($where)->limit(1)->get('node')->result_array();
