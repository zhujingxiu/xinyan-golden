...
{
    protected $activity_table = "worker_activity";
    function insert_worker_activity($msg,$worker_id)
    {
        $this->db->insert($this->activity_table,array(
            'worker_id' => $worker_id,
            'content' => $msg,
            'addtime' => time()
        ));
    ...
    function get_activity_list($limit){
        $this->db->select()
            ->where(array('worker_id'=>$this->ion_auth->get_user_id()))
